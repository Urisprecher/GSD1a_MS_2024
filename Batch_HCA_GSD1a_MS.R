...
gc()
## main
detect_batch_effects <- function(data_folder,
  ...
  required_packages <- c("ggplot2", "pheatmap", "vegan", "clr", "RColorBrewer")
  ...
  # Prompt user for the results folder name if required
  ...
  # Create main output folder and subdirectories
  create_directory(output_folder)
  for (subdir in subdirs) {
    create_directory(file.path(output_folder, subdir))
  }
  
  # Read data and meta files with error handling
  ...
  # Optionally prompt for row names
  if (rowname_prompt) {
    tryCatch({
  ...
  # Prompt user to choose treatment and batch parameters
  ...
  if (batch_param_prompt) {
    trt_param <- get_meta_param(meta, "treatment parameter")
  ...
  # Proceed with log-ratio transformation
  tryCatch({
  ...
  tryCatch({
    ...
    # Check if PCA plot is not empty
    if (!is.null(data.pca.plot)) {
      # Print PCA plot
      print(data.pca.plot)
      
      # Create output file path for PCA plot
      pca_plot_file <- file.path(output_folder, "pca_plot.png")
      
      # Save PCA plot with higher quality and size
  ...
  # Create subdirectories for box plots, linear regression summaries, RLE plots, heatmap, and var calc
  # Already created earlier
  
  # Call analyze_df function with error handling
  tryCatch({
  ...
  # Heatmap Generation with Improved Colors
  tryCatch({
    # Scale the data
  ...
  tryCatch({
    # Calculate variation explained for each feature
    variation_explained <- apply(t(data.clr), 1, function(x) var(x) / sum(var(x)))
    
    # Sort the features based on their variation explained
    ...
    # Subset the data for top features
    ...
    # Generate second Heatmap for top features
  ...
  # Variance Plotting
  tryCatch({
    # Variance calculation before batch correction
...
data_folder <- "D:/MiguelW12/Documents/gsd1a_batch"

detect_batch_effects(data_folder,
                     output_folder_prompt = TRUE,
                     rowname_prompt = TRUE,
                     batch_param_prompt = TRUE, 
                     num_features = 20)
